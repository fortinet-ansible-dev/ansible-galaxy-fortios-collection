...
def switch_controller_location(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('switch_controller', 'location', filtered_data, vdom=vdom)
        current_data = fos.get('switch_controller', 'location', vdom=vdom, mkey=mkey)
...
def fortios_switch_controller(data, fos, check_mode):
    ...
    fos.do_member_operation('switch_controller_location')
    if data['switch_controller_location']:
        resp = switch_controller_location(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('switch_controller_location'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["switch_controller_location"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_controller_location"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "switch_controller_location")
